...
frappe.ui.form.on("Sales Invoice", {
	...
	async fetch_project_retention(frm) {
		...
		if (cint(project.enable_retention)) {
			...
			if (!frm.doc.retention_account && frm.doc.company) {
				...
				await frm.set_value(
					"retention_account",
					company.message && company.message.default_retention_receivable_account
				);
			}
	...
	calculate_retention(frm) {
		...
		frm.set_value({
			...
			net_receivable_amount: flt(total - retention_outstanding_amount, precision("net_receivable_amount")),
		});
